...
import '../../data/hive/chat_history.dart';
...
class ChatProvider extends ChangeNotifier {
  ...
  Future<List<Message>> loadMessagesFromDB({required String chatId}) async {
    ...
    await Hive.openBox('$kChatMessagesBox$chatId');

    final messageBox = Hive.box('$kChatMessagesBox$chatId');
  ...
  Future<void> deletChatMessages({required String chatId}) async {
    ...
    if (!Hive.isBoxOpen('$kChatMessagesBox$chatId')) {
      // open the box
      await Hive.openBox('$kChatMessagesBox$chatId');

      // delete all messages in the box
      await Hive.box('$kChatMessagesBox$chatId').clear();

      // close the box
      await Hive.box('$kChatMessagesBox$chatId').close();
    } else {
      // delete all messages in the box
      await Hive.box('$kChatMessagesBox$chatId').clear();
  ...
  Future<void> prepareChatRoom({
    ...
    if (!isNewChat) {
      // Existing chat: load the chat messages from the database
      final chatHistory = await loadMessagesFromDB(chatId: chatID);

      // Clear inChatMessages
      _inChatMessages.clear();
      ...
      // New chat: clear inChatMessages and generate a new chat ID
      _inChatMessages.clear();

      // Generate a new chat ID for the new chat
      final newChatId = const Uuid().v4(); // Generates a unique chat ID
  ...
  Future<void> sentMessage({
    ...
    final messagesBox = await Hive.openBox('$kChatMessagesBox$chatId');
  ...
  static initHive() async {
    ...
    Hive.init(dir.path);
    await Hive.initFlutter(kGeminiDB);
    ...
    if (!Hive.isAdapterRegistered(0)) {
      ...
      await Hive.openBox<ChatHistory>(kChatHistoryBox);
    }

    if (!Hive.isAdapterRegistered(2)) {
      Hive.registerAdapter(SettingsAdapter());
      await Hive.openBox<Settings>(kSettingsBox);
    }
  }
}
